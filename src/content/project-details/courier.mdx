...
<Callout title="Message queuing is not perfect">
  The previous message isn't reliably triggered after the first one is sent. That's a state-machine bug in the queue/receiver path — the first thing I'd fix on a rewrite.
</Callout>
